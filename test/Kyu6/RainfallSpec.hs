module Kyu6.RainfallSpec where
import Kyu6.Rainfall   
...
towns = ["Rome", "London", "Paris", "NY", "Vancouver", "Sydney", "Bangkok", "Tokyo",
          "Beijing", "Lima", "Montevideo", "Caracas", "Madrid", "Berlin", "Lisboa"]

shouldBeFuzzy :: Double -> Double -> Expectation
shouldBeFuzzy act expec =
    when (isNaN(act) || abs(act - expec) > 1e-2) $ expectationFailure msg
  where msg = "abs(actual - expected) must be <= 1e-2. Expected was " ++ show expec ++ " but got: " ++ show act

testMean :: String -> String -> Double -> Spec
testMean twn dt u = 
    it (printf "should return mean for town: %s " (show twn)) $
        mean twn dt `shouldBeFuzzy` u
testVar :: String -> String -> Double -> Spec
testVar twn dt u = 
    it (printf "should return variance for town: %s " (show twn)) $
        variance twn dt `shouldBeFuzzy` u

spec :: Spec
spec = do
    describe "mean with data0" $ do
        
        testMean "London" data0 51.199999999999996
        testMean "Beijing" data0 52.416666666666664

    describe "variance with data0" $ do
        
        testVar "London" data0 57.428333333333374
        testVar "Beijing" data0 4808.37138888889
