...
spec = do
  describe "pattern" $ do
    ...
    it "returns a string where all lines start with a '1'" $
      property $ \(Positive n) ->
    ...
    it "does not end with a newline character" $
      property $ \(Positive n) ->
        pattern n `shouldSatisfy` ('\n' /=) . last

    it "returns a string where all lines end with the correct number" $
      property $ \(Positive n) ->
        let dropped = zipWith drop [2 ..] . drop 1 . lines
            numberP = and . zipWith (==) (map show [2 ..]) . dropped
         in pattern n `shouldSatisfy` numberP

    it "has the correct amount of stars per line" $
      property $ \(Positive n) ->
