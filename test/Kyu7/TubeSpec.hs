...
import Kyu7.Tube (calculator, Decision(..))
...
spec = do
  describe "calculator" $ do 
    forM_ [ (5, 6, 1,    Bus,  "The bus should win this time!")
    ...
    it "returns Walk if the distance is small" $ property $
      forAll (choose (1e-1,0.83)) $ \d -> \(Positive b) (Positive w) ->
        calculator d b w `shouldBe` Walk
      
    it "returns Bus if the distance is huge" $ property $
