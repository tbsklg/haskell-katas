...
spec = describe "noRepeat" $ do
  it "works for strings with a single character" $ property $ \c ->
  ...
  it "returns the first character for strings without duplicates" $ property $ 
    forAll (sublistOf ['a'..'z']) $ \xs ->
      noRepeat xs `shouldBe` head xs
  
  it "finds the single non-duplicates character" $ property $ 
    forAll (sublistOf ['a'..'z'] >>= shuffle >>= duplicatify) $ \(x,xs) ->
      noRepeat xs `shouldBe` x
  
  it "returns a character that has no duplicate" $ property $ 
